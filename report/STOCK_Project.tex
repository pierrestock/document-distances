...
An important breakthrough in the last years has been the introduction of the word2vec model by Mikolov et al. \cite{mikolov}. The subsequent freely available model generates low-dimensional word embeddings using a 2-layers shallow neural network that has been trained on approximately 100 billion words. The authors demonstrated that the model learned high-quality word representations that are able to capture precise syntactic and semantic meanings: for instance, the relationship vec(\textit{king}) - vec(\textit{man}) + vec(\textit{women}) has an algebraic meaning since the obtained vector is very close to vec(\textit{queen}). 

More recently, Quoc and Mikolov \cite{quoc} introduced doc2vec, an unsupervised algorithm that learns fixed-length feature representations from variable-length pieces of texts, such as sentences, paragraphs, and documents. However, their model has to be re-trained depending on the dataset we want to study. 
...
The gains to using GPUs appears clearly. Suppose the size of the dictionary is $n = $10,000 and we want to compute all the 4,950 pairwise similarities between $d = 100$ documents. Then, we could iterate in place in the GPU, which will be very efficient for the highly parallel task of repeatedly performing elementary operations between matrixes of size $n \times d(d-1)/2$. We implemented this algorithm\footnote{Available at \url{https://github.com/pierrestock/document-distances}}  to perform all pairwise comparisons between two datasets of documents in Python using Tensorflow and ran it on a ?? card. Further documentation is available online. 
...
The authors trained a 2-layers shallow neural network on approximately 100 billion words to predict the context of a given word, e.g. to predict of to words often appear nearby. The embedding is obtained as the weights of the Interestingly, the model learned high-quality word representations that are able to capture precise syntactic and semantic meanings. 
...
\textbf{Distance matrix.} We used the freely-available pre-trained word2vec model\footnote{Available at \url{https://code.google.com/archive/p/word2vec/ }} to compute once for all the distance matrix using the $L_p$ norm for different values of $p$. By default and unless contrary indication, we use $p = 2$. 
...
\textbf{Implementation.} We implemented Sinkhorn's algorithm in Python 3.5 using Tensorflow 0.12 and ran it on ??
...
\textbf{Evaluating the results.} We evaluated the $k$NN performance of our algorithm for $k = 1$ and $k = 5$ on the test sets. 

\subsection{Influence of the Regularization Parameter $\lambda$}

\subsection{Influence of the Embedding Metric}

\subsection{Unbalanced Problem}
...
Possible extensions of this work include improving the fixed dictionary by including celebrity names for example and computing its optimal size and testing the performance of this algorithm on other datasets or other tasks like. 
...
An other limitation is that BoW features loose the ordering of the words in the documents, ordering that is only partially retrieved using the word2vec embedding by associating short distances to words that are similar, \textit{i.e.} that often appear nearby in the same context. 

for two reasons: (i) since some of the quantities manipulated in the proposed algorithms have an order of e?1/? (in particular Gibbs distributions denoted ? in the following) they become smaller than machine precision whenever the regularization ? is small; (ii) more importantly, even if the first issue is taken care of by carrying out computations in the log domain, the convergence speed of iterative projec- tion methods degrades significantly as ? ? 0. 
